...
export class WeatherManager {
    ...
    getBackgroundColor(): string {
        ...
        if (code === 0) return '#FFFACD'; // Ensoleillé - jaune pâle
        if (code <= 3) return '#FFDAB9'; // Nuageux - orange clair
        if (code <= 48) return '#F5F5DC'; // Brouillard - blanc cassé
        if (code <= 65) return '#8FBC8F'; // Pluie - vert doux
    ...
    getManualTimeOverride(): boolean {
        return this.manualTimeOverride;
    }

    getManualWeatherOverride(): boolean {
